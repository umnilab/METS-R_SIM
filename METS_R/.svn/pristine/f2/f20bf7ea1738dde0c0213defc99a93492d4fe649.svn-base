...
File: ObjectGraphToDoubleGraphAdapter.java 
...
final class ObjectGraphToDoubleGraphAdapter<N extends GObject> implements DoubleGraph<N> {

  private final ObjectGraph<N, Double> g;

  ObjectGraphToDoubleGraphAdapter(ObjectGraph<N, Double> g) {
    this.g = g;
  }
  ...
  @Override
  public boolean addEdge(GNode<N> src, GNode<N> dst, double data) {
    return g.addEdge(src, dst, data);
  }

  @Override
  public boolean addEdge(GNode<N> src, GNode<N> dst, double data, byte flags) {
    return g.addEdge(src, dst, data, flags);
  }

  @Override
  public double getEdgeData(GNode<N> src, GNode<N> dst) {
    Double data = g.getEdgeData(src, dst, MethodFlag.ALL, MethodFlag.NONE);
    return data == null ? -1 : data;
  }

  @Override
  public double getEdgeData(GNode<N> src, GNode<N> dst, byte flags) {
    Double data = g.getEdgeData(src, dst, flags, MethodFlag.NONE);
    return data == null ? -1 : data;
  }

  @Override
  public double setEdgeData(GNode<N> src, GNode<N> dst, double d) {
    Double data = g.setEdgeData(src, dst, d);
    return data == null ? -1 : data;
  }

  @Override
  public double setEdgeData(GNode<N> src, GNode<N> dst, double d, byte flags) {
    Double data = g.setEdgeData(src, dst, d, flags);
    return data == null ? -1 : data;
  }
