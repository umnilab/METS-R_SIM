...
public class MetisGraph {
	...
	/**
	 * Get the maximal adjsum(the sum of the outgoing edge weights of a node) among
	 * all the nodes
	 */
	...
	/**
	 * Compute the sum of the weights of all the outgoing edges for each node in the
	 * graph
	 */
	...
	class ComputeCutClosure implements LambdaVoid<GNode<MetisNode>> {
		...
		public void call(GNode<MetisNode> node) {
			node.map(new Lambda2Void<GNode<MetisNode>, GNode<MetisNode>>() {
				public void call(GNode<MetisNode> neighbor, GNode<MetisNode> node) {
					if (neighbor.getData(MethodFlag.NONE).getPartition() != node.getData(MethodFlag.NONE)
							.getPartition()) {
						int edgeWeight = (int) graph.getEdgeData(node, neighbor, MethodFlag.NONE);
						cut = cut + edgeWeight;
					}
	...
	/**
	 * Verify if the partitioning is correctly performed by checking the internal
	 * maintained graph cut is same as the real graph cut
	 */
	public boolean verify() {
		if (mincut.get() == computeCut()) {
			return true;
		} else {
			System.err.println("mincut is computed wrongly:" + mincut.get() + " is not equal " + computeCut());
			return false;
		}
	}
