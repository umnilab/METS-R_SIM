	...
	private void refineOneNode(final MetisGraph metisGraph, GNode<MetisNode> n) {
		...
		if (nodeData.getEdegree() >= nodeData.getIdegree()) {
			...
			for (k = 0; k < nodeData.getNDegrees(); k++) {
				...
				if (metisGraph.getPartWeight(to, MethodFlag.CHECK_CONFLICT) + vwgt <= maxwgts[to] + ffactor * gain
						&& gain >= 0)
					break;
			}
			...
			if (nodeData.partEd[k] - nodeData.getIdegree() > 0)
				...
				if (from_weight >= maxwgts[from] || itpwgts[from] * (to_weight + vwgt) < itpwgts[to] * from_weight)
					j = 1;
			}
			if (j == 0)
				return;
