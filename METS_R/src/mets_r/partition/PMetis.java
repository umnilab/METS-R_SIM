...
public class PMetis {
	...
	/**
	 * TotalPartWeights: This is an array containing "nparts" floating point
	 * numbers. For partition i , totalPartitionWeights[i] stores the fraction of
	 * the total weight that should be assigned to it. See tpwgts in manual of
	 * metis.
	 * 
	...
	private static MetisGraph[] splitGraph(MetisGraph metisGraph) {
		...
		IntGraph<MetisNode> empty = new MorphGraph.IntGraphBuilder().backedByVector(true).directed(true).serial(true)
				.create();
		...
		graph.map(new LambdaVoid<GNode<MetisNode>>() {
			...
			public void call(GNode<MetisNode> node) {
				MetisNode nodeData = node.getData();
				GNode<MetisNode> newNode = subGraphs[nodeData.getPartition()].getGraph()
						.createNode(new MetisNode(subGraphNodeNum[nodeData.getPartition()], nodeData.getWeight()));
				...
				node.map(new Lambda2Void<GNode<MetisNode>, GNode<MetisNode>>() {
					public void call(GNode<MetisNode> neighbor, GNode<MetisNode> node) {
						...
						if (!nodeData.isBoundary() || nodeData.getPartition() == neighborData.getPartition()) {
							...
							nodeData.getSubGraphMap().getData()
									.setAdjWgtSum(nodeData.getSubGraphMap().getData().getAdjWgtSum() - edgeWeight);
						}
					}
				}, node);
			}
