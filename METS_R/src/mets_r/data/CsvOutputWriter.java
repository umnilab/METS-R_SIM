 ...
 * represented by that line. Each additional field from the third column to the
 * end of the line is specific to the type of data being represented by that
 * line.
 * 
...
public class CsvOutputWriter implements DataConsumer {
	...
	/**
	 * Creates the CSV output writer object using a default file name and not
	...
	/**
	 * Creates the CSV output writer object to place output from the simulation
	...
	/**
	 * Creates a buffered writer for saving the CSV lines to disk and opens it. Any
	...
	private void startNextOutputFile() throws IOException {
		...
		String currentEnd = "." + this.fileSeriesNumber + "." + GlobalVariables.CSV_DEFAULT_EXTENSION;
		String nextEnd = "." + (this.fileSeriesNumber + 1) + "." + GlobalVariables.CSV_DEFAULT_EXTENSION;
	...
	/**
	 * Writes the given tick snapshot to the output file after converting it to a
	 * series of CSV lines. The buffered writer is flushed at the end so any cached
	 * content is immediately written out to disk.
	 * 
	...
	/**
	 * Returns the CSV representation of the given tick snapshot as an array of
	 * strings.
	 * 
	...
	public static String createVehicleLine(VehicleSnapshot vehicle) {
		...
		return (id + "," + prev_x + "," + prev_y + "," + x + "," + y + "," + speed + "," + vehicleClass + "," + roadID);
	}
