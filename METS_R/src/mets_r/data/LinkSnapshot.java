...
class LinkSnapshot {
	...
	/**
	 * Number of vehicles currently on the road, including the junction
	 */
	final int nVehicles;
	...
	LinkSnapshot(int id, double speed, int nVehicles, double energy, int flow) {
		...
		if (nVehicles < 0) {
			throw new ValueException("No. of vehicles on road is negative for " + id);
		}

		if (flow < 0) {
			throw new ValueException("No. of Flows is negative for " + id);
		}
