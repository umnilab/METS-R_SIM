...
File: ObjectGraphToIntGraphAdapter.java 
...
final class ObjectGraphToIntGraphAdapter<N extends GObject> implements IntGraph<N> {

  private final ObjectGraph<N, Integer> g;

  ObjectGraphToIntGraphAdapter(ObjectGraph<N, Integer> g) {
    this.g = g;
  }
  ...
  public int getEdgeData(GNode<N> src, GNode<N> dst) {
    Integer data = g.getEdgeData(src, dst, MethodFlag.ALL, MethodFlag.NONE);
    return data == null ? -1 : data;
  }

  @Override
  public int getEdgeData(GNode<N> src, GNode<N> dst, byte flags) {
    Integer data = g.getEdgeData(src, dst, flags, MethodFlag.NONE);
    return data == null ? -1 : data;
  }

  @Override
  public int setEdgeData(GNode<N> src, GNode<N> dst, int d) {
    Integer data = g.setEdgeData(src, dst, d);
    return data == null ? -1 : data;
  }

  @Override
  public int setEdgeData(GNode<N> src, GNode<N> dst, int d, byte flags) {
    Integer data = g.setEdgeData(src, dst, d, flags);
    return data == null ? -1 : data;
  }
