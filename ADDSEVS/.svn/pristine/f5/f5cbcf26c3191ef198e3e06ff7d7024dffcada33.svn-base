...
class OrderedLeaf<T> implements OrderableWorklist<T> {
  private final PriorityQueue<T> queue;
  private final Comparator<T> comp;

  public OrderedLeaf(Comparator<T> comp, Maker<T> maker, boolean needSize) {
    this.comp = comp;
    queue = new PriorityQueue<T>(111, comp);
  }

  @Override
  public Worklist<T> newInstance() {
    return new OrderedLeaf<T>(comp, null, false);
  }
