 ...
 * program (such as an output file saving code, a network connection code,
 * or some other local form of analysis) can use to easily gather the results
 * of the model without intrusively interrupting or impacting its operation. 
 * 
...
//    private EnergyCalculator energeCalculator;
     ...
     * collector being created within the program.  All references to
     * the collector should be made through DataCollector.getInstance()
     * which will create the object the first time it is needed.
     */
     ...
     * and reset any data structures used for buffering the data.  Any
     * registered data consumers are also started (and possibly told to
...
//        this.energeCalculator = new EnergyCalculator();
     ...
     * simulation and tells each of the data consumers to shut down once they
     * have processed what remains of the buffer.  The collection system
     * continues to exist to allow the registered consumers to finish working
     * upon what is left in the buffer.  Once all the items in the buffer have
     * been cleared, the collection system can finalize stopping.
     */
     ...
     * system only halts the acceptance of new data and does not dispose of
     * any data structures or halt any other operations which may prevent it
     * from continuing to collect data in the future.  This means to pause the
     * system, this method only has to change the value of two boolean flags.
     */
    ...
    /**
     * If the data collection is already running and has been paused,
     * this method will tell the system to resume accepting new data.
     */
    ...
    /**
     * The data collection system prepares to begin collecting data into a
     * new tick snapshot.  All the data collected during a single tick of
     * the simulation is collected into one snapshot object capturing the
     * state of the model during that particular tick.
     * 
    ...
    /**
     * The data collection system finishes collecting data for the current
     * tick and adds the object to the data buffer so that data consumers
    ...
    public void registerDataConsumer(DataConsumer consumer) {
        ...
        if (this.isCollecting()) {
            try {
                ...
                // TODO: Handle the consumer not able to start consuming
            }
        }
    }
    ...
    /**
     * Removes the given data consumer from the list of registered data
     * consumers in the system.  Each object in the program which needs to
     * read data from the buffer should register a data consumer and should
     * remove the consumer from the list when it has finished.
     * 
    ...
    /**
     * Returns the tick number of the oldest item in the buffer.
     * If no items have ever been stored in the buffer, a negative
     * value is returned.
     * 
    ...
    /**
     * Returns the tick snapshot from the buffer matching the given number
     * if possible.  If no matching tick snapshot is found, the next item
     * in the list with the closest (greater) tick number is returned.
     * 
    ...
    /**
     * This method is called periodically while the buffer has contents
     * to check if any of the oldest items still in the buffer can be
     ...
     * debugging has been enabled (via a constant boolean flag in the
     * DataCollector class).  If debugging is not enabled, the message
     * is ignored.
     * 
    ...
    /**
     * Logs the given message to the standard output if data collection
     * debugging has been enabled (via a constant boolean blag in the
     * DataCollector class) with the given custom string in the prefix.
     * If debugging is not enabled, the message is ignored.
     * 
